...
export function getDurationSeconds(config: PomodoroConfig, stage: TimerStage): number {
  const minutes = {
...
export function getNextStage(
  ...
  const isTimeForLongBreak = completedPomodoros % config.longBreakInterval === 0
...
export function useCountdown(config: PomodoroConfig): CountdownController {
  ...
  const [remainingSeconds, setRemainingSeconds] = useState(() => getDurationSeconds(config, TimerStage.POMODORO))
